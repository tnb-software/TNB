...
public final class ResourceParsers {
    public static boolean isPodReady(Pod pod) {
        return pod.getStatus().getContainerStatuses().size() > 0 && pod.getStatus().getContainerStatuses().stream()
    ...
    public static boolean hasPodRestartedAtLeastNTimes(Pod pod, int n) {
        return pod.getStatus().getContainerStatuses().size() > 0 && pod.getStatus().getContainerStatuses().stream().anyMatch((x) -> {
            return x.getRestartCount() >= n;
        });
    }
