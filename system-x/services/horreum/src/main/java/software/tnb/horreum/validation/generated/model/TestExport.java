...
public class TestExport {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(variables, missingDataRules, experiments, actions, subscriptions, datastore, id, name, folder, description, datastoreId, timelineLabels, timelineFunction, fingerprintLabels, fingerprintFilter, compareUrl, transformers, notificationsEnabled, access, owner);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TestExport.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TestExport is not found in the empty JSON string", TestExport.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TestExport.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TestExport` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("missingDataRules") != null && !jsonObj.get("missingDataRules").isJsonNull()) {
        ...
        if (jsonArraymissingDataRules != null) {
          ...
          if (!jsonObj.get("missingDataRules").isJsonArray()) {
            throw new IllegalArgumentException(String.format("Expected the field `missingDataRules` to be an array in the JSON string but got `%s`", jsonObj.get("missingDataRules").toString()));
          }
