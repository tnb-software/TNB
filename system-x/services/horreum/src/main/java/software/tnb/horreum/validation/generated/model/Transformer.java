  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Transformer.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Transformer` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("targetSchemaUri") != null && !jsonObj.get("targetSchemaUri").isJsonNull()) && !jsonObj.get("targetSchemaUri").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `targetSchemaUri` to be a primitive type in the JSON string but got `%s`", jsonObj.get("targetSchemaUri").toString()));
      }
