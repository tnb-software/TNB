...
public class OpenshiftDocling extends Docling implements OpenshiftDeployable, WithName {
    private static final Logger LOG = LoggerFactory.getLogger(OpenshiftDocling.class);
    ...
    public void closeResources() {
        ...
        if (portForward != null) {
            try {
                ...
                LOG.warn("Unable to close Docling port forward", e);
            }
        }
        if (client != null) {
            try {
    ...
    public void create() {
        ...
        // Add volume and volume mount for model cache
        // Mount writable volume directly at RapidOCR models directory
        List<io.fabric8.kubernetes.api.model.Volume> volumes = new LinkedList<>();
        ...
        List<io.fabric8.kubernetes.api.model.VolumeMount> volumeMounts = new LinkedList<>();
