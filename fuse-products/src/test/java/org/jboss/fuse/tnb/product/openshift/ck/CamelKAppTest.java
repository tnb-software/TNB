...
public class CamelKAppTest extends CamelKTestParent {
    ...
    private CamelKIntegrationBuilder ckib() {
        return new CamelKIntegrationBuilder(name());
    }
    ...
    public void shouldContainYamlIntegrationSourceCodeTest() {
        Path filePath = null;
        try {
            filePath = Paths.get(this.getClass().getResource("/org/jboss/fuse/tnb/product/openshift/ck/Hello.yaml").toURI());
        } catch (Exception e) {
    ...
    public void shouldProcessPropertiesTest() {
        ...
        assertThat(config.getValue()).isEqualTo(name());
    ...
    public void shouldNotBeReadyWhenPodIsNotReadyTest() {
        ...
        OpenshiftClient.get().pods().create(new PodBuilder()
            .withNewMetadata()
            .withLabels(Map.of("camel.apache.org/integration", name()))
            .withName("camel-k")
            .endMetadata()
            .withNewStatus()
    ...
    public void shouldCreateKameletBindingTest() {
        CamelKApp app = new CamelKApp(new KameletBindingBuilder().withNewMetadata().withName(name()).endMetadata().build());
        executor.submit(app::start);

        Awaitility.await().atMost(10, TimeUnit.SECONDS).untilAsserted(() -> assertThat(kameletBindingClient.withName(name()).get()).isNotNull());
    }

    @Test
    public void shouldRemoveKameletBindingTest() {
        KameletBinding kb = new KameletBindingBuilder().withNewMetadata().withName(name()).endMetadata().build();
        kameletBindingClient.create(kb);
        CamelKApp app = new CamelKApp(kb);

        app.stop();
        assertThat(OpenshiftClient.get().adapt(CamelKClient.class).v1().integrations().withName(name()).get()).isNull();
    }
